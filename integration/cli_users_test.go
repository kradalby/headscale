...
func TestUserCreateCommand(t *testing.T) {
	...
	// Creating a user with a name that already exists must fail and must not
	// leave a second user behind.
	...
	require.ErrorContains(t, err, "already exists", "creating a duplicate user must fail")

	// destroy with neither --name nor --identifier must be rejected.
	_, err = headscale.Execute([]string{
		"headscale",
		"users",
		"destroy",
	...
	require.ErrorContains(t, err, "--name or --identifier flag is required")
}
