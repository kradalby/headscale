...
//
// The upstream axum example does not expose control_server_url or
// hostname on the CLI, so our Dockerfile patches examples/axum/main.rs
// to read them from the TS_CONTROL_URL and TS_HOSTNAME environment
// variables. The key file (-c) is created on first run automatically.
func (t *TailscaleRustInContainer) buildEntrypoint() []string {
	...
	// Env vars consumed by the patched axum example.
	commands = append(commands,
		fmt.Sprintf(`export TS_CONTROL_URL=%q`, t.headscaleURL),
		fmt.Sprintf(`export TS_HOSTNAME=%q`, t.hostname),
		"export TS_RS_EXPERIMENT=this_is_unstable_software",
	)

	axumCmd := "/usr/local/bin/axum -c /tmp/tsrs-keys.json"
	if t.authKey != "" {
		axumCmd += " -k " + t.authKey
	}
