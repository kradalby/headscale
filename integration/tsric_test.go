...
func TestTailscaleRustAxum(t *testing.T) {
	...
	err = scenario.CreateHeadscaleEnv(
		...
		// Use public DERP relays. The embedded DERP server has compatibility
		// issues with tailscale-rs (port/TLS mismatch), so we use Tailscale's
		// public relay infrastructure which both clients support.
		hsic.WithPublicDERP(),
		hsic.WithoutTLS(),
	)
