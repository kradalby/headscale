...
func TestNodeExpireFlagsCommand(t *testing.T) {
	...
	updated := assertJSONRoundtrip[*v1.Node](t, headscale, []string{
		"headscale", "nodes", "expire",
		"--identifier", nodeID,
		"--expiry", future,
		"--output", "json",
	})
	assert.False(t, updated.GetExpiry().AsTime().IsZero(), "expiry should be set to the future time")
	assert.True(t, updated.GetExpiry().AsTime().After(time.Now()), "expiry should be in the future")

	// Disable expiry entirely.
	disabled := assertJSONRoundtrip[*v1.Node](t, headscale, []string{
		"headscale", "nodes", "expire",
		"--identifier", nodeID,
		"--disable",
		"--output", "json",
	})
	assert.True(t, disabled.GetExpiry().AsTime().IsZero(), "expiry should be cleared by --disable")
}

// TestNodeDeleteErrors covers the validation paths of `nodes delete`: the
// required --identifier flag and a non-existent node.
func TestNodeDeleteErrors(t *testing.T) {
	IntegrationSkip(t)

	scenario, headscale := setupCLIScenario(t, "cli-nodedeleteerr", []string{"user1"}, 0)
	defer scenario.ShutdownAssertNoPanics(t)

	// Missing required --identifier flag.
	_, err := headscale.Execute([]string{"headscale", "nodes", "delete", "--force"})
	...
	require.Error(t, err, "deleting a non-existent node must fail")
}
...
func TestNodeTagCommand(t *testing.T) {
	...
	scenario, headscale := setupCLIScenario(t, "cli-nodetag", []string{"user1"}, 1)
	defer scenario.ShutdownAssertNoPanics(t)
	...
	// Set two tags. The response node should carry both tags.
	...
	assert.ElementsMatch(t, []string{"tag:test1", "tag:test2"}, tagged.GetTags())
