...
// Empty changes (see [change.Change.IsEmpty]) carry no work for any
// recipient: they are dropped here so they never create pending entries,
// in-flight work, or recipient processing. A real PeersRemoved change is
// not empty and still triggers deletion cleanup in addToBatch.
//
// See https://github.com/juanfont/headscale/issues/3417.
func (b *Batcher) AddWork(r ...change.Change) {
	...
	if len(r) == 1 {
		if r[0].IsEmpty() {
			mapperChangesDropped.WithLabelValues("empty").Inc()
			return
		}
	...
	// Multi-change: drop empties without mutating the caller's slice.
	filtered := make([]change.Change, 0, len(r))
	for _, ch := range r {
		if ch.IsEmpty() {
			mapperChangesDropped.WithLabelValues("empty").Inc()
			continue
