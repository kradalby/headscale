...
// TestBatcherMultiConnection tests the multi-channel batcher's ability to handle
// multiple concurrent connections for the same node and ensure all connections
// receive updates properly.
func TestBatcherConnectedMapBug(t *testing.T) {
	t.Parallel()

	// Use the standard setup like other tests
	testData, cleanup := setupBatcherWithTestData(t, NewBatcherAndMapper, 1, 3, 10)
	defer cleanup()
	...
	// Add nodes
	channels := make([]chan *tailcfg.MapResponse, len(nodes))
	for i, node := range nodes {
		channels[i] = make(chan *tailcfg.MapResponse, 10)
		err := batcher.AddNode(node.n.ID, channels[i], tailcfg.CapabilityVersion(55))
		require.NoError(t, err)
	}

	// Check Debug method shows all nodes as connected  
	debugInfo := batcher.(*testBatcherWrapper).Batcher.(*LockFreeBatcher).Debug()
	for _, node := range nodes {
		info, exists := debugInfo[node.n.ID]
		require.True(t, exists, "Node %d should exist in debug info", node.n.ID)
		require.True(t, info.Connected, "Node %d should be connected in debug info", node.n.ID)
		require.Equal(t, 1, info.ActiveConnections, "Node %d should have 1 active connection", node.n.ID)
	}

	// Disconnect first node
	removed := batcher.RemoveNode(nodes[0].n.ID, channels[0])
	require.True(t, removed, "Node should be successfully removed")

	// Check Debug method shows first node as disconnected immediately
	debugInfo = batcher.(*testBatcherWrapper).Batcher.(*LockFreeBatcher).Debug()
	info, exists := debugInfo[nodes[0].n.ID]
	require.True(t, exists, "Disconnected node should still exist in debug info")
	require.False(t, info.Connected, "Node should be marked as disconnected in debug info")
	require.Equal(t, 0, info.ActiveConnections, "Disconnected node should have 0 active connections")

	// Other nodes should still be connected - get fresh debug info
	debugInfo = batcher.(*testBatcherWrapper).Batcher.(*LockFreeBatcher).Debug()
	for i := 1; i < len(nodes); i++ {
		info, exists := debugInfo[nodes[i].n.ID]
		require.True(t, exists, "Node %d should exist in debug info", nodes[i].n.ID)
		require.True(t, info.Connected, "Node %d should still be connected", nodes[i].n.ID)
		require.Equal(t, 1, info.ActiveConnections, "Node %d should have 1 active connection", nodes[i].n.ID)
	}

	// Reconnect the first node
	newChannel := make(chan *tailcfg.MapResponse, 10)
	err := batcher.AddNode(nodes[0].n.ID, newChannel, tailcfg.CapabilityVersion(55))
	require.NoError(t, err)
	
	// Update the channels array to use the new channel for node 0
	channels[0] = newChannel

	// Check Debug method shows first node as connected again
	debugInfo = batcher.(*testBatcherWrapper).Batcher.(*LockFreeBatcher).Debug()
	{
		info, exists := debugInfo[nodes[0].n.ID]
		require.True(t, exists, "Reconnected node should exist in debug info")
		require.True(t, info.Connected, "Node should be connected again in debug info")
		require.Equal(t, 1, info.ActiveConnections, "Reconnected node should have 1 active connection")
	}

	// Now simulate the integration test scenario: rapid disconnect/reconnect of ALL nodes
	// This replicates what happens in TestPingAllByIPMany where nodes try to reconnect
	t.Log("Simulating rapid disconnect/reconnect of all nodes...")
	
	// First, disconnect all nodes rapidly
	for i, node := range nodes {
		removed := batcher.RemoveNode(node.n.ID, channels[i])
		require.True(t, removed, "Node %d should be successfully removed", node.n.ID)
	}
	
	// Brief pause to simulate network timing like in integration tests
	time.Sleep(10 * time.Millisecond)
	
	// Reconnect all nodes rapidly (like integration test does)
	newChannels := make([]chan *tailcfg.MapResponse, len(nodes))
	for i, node := range nodes {
		newChannels[i] = make(chan *tailcfg.MapResponse, 10)
		err := batcher.AddNode(node.n.ID, newChannels[i], tailcfg.CapabilityVersion(55))
		require.NoError(t, err, "Node %d should reconnect successfully", node.n.ID)
	}
	
	// Now check if ALL nodes show as connected (this is what was failing in integration tests)
	debugInfo = batcher.(*testBatcherWrapper).Batcher.(*LockFreeBatcher).Debug()
	connectedCount := 0
	for _, node := range nodes {
		info, exists := debugInfo[node.n.ID]
		require.True(t, exists, "Node %d should exist in debug info after reconnection", node.n.ID)
		if info.Connected {
			connectedCount++
			require.Equal(t, 1, info.ActiveConnections, "Connected node %d should have 1 active connection", node.n.ID)
		} else {
			t.Logf("Node %d is still shown as disconnected", node.n.ID)
		}
	}
	
	t.Logf("After rapid reconnection: %d/%d nodes connected", connectedCount, len(nodes))
	require.Equal(t, len(nodes), connectedCount, "All nodes should be connected after rapid reconnection")
}
